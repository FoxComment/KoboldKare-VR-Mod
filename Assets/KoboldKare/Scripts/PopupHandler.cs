...
public class PopupHandler : MonoBehaviour
{
    ...
    private GameObject canvas
    {
        get
        {
            ...
            //internalCanvas.AddComponent<UnityEngine.XR.Interaction.Toolkit.UI.TrackedDeviceGraphicRaycaster>();
            c.renderMode = RenderMode.WorldSpace;
            c.sortingOrder = 2;  //used to be 2
    ...
    public Popup SpawnPopup(string name, bool solo = true, string title = default, string description = default, Sprite icon = null)
    {
        ...
        if (popup != null)
        {
            ...
            popup.GetComponent<Canvas>().worldCamera = FoxVRLoader.GetUICamera();
            popup.transform.localScale = FoxVRLoader.GetPauseScreenTransform().lossyScale;
            popup.transform.position = FoxVRLoader.GetPauseScreenTransform().position;
            popup.transform.rotation = FoxVRLoader.GetPauseScreenTransform().rotation;
            print(FoxVRLoader.GetPauseScreenTransform().position);
